...
import Hakyll
import Site.Meta hiding (postContext)
...
archiveContext :: Pattern -> Context String -> Context a
archiveContext pattern postContext =
  listField "years" (year <> posts) (groupedArchives pattern)
  where
    year  = field "year" (return . show . fst . itemBody)
    posts = Context $ \k _ i ->
              if k == "posts"
                then return $ ListField postContext (snd . itemBody $ i)
                else empty
